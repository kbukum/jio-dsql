    ...
    public static <E> void criteriaToQuery(
        ...
        if(selectJoiner != null) {
            if(criteria.getProjection() != null) {
                String select = HqlUtilTest.selectForListByProjection(criteria, criteria.getProjection(), null, elements, groupJoiner);
                if(!Strings.isEmptyOrNull(select)) {
                    selectJoiner.add(select);
                }
            }
        }
        if(criteria.getRestrictions() != null && criteria.getRestrictions().size() > 0) {
            HqlUtilTest.generateRestrictions(
                    criteria,
        ...
        if(criteria.getJoins() != null && criteria.getJoins().size() > 0) {
            for(Map.Entry<String, CriteriaJoin<E>> joinEntry: criteria.getJoins().entrySet()) {
                String joinToString = HqlUtilTest.joinToString(joinEntry.getValue());
            ...
            for(Order order :criteria.getOrders() ) {
                ...
                if(order.isAlias()) {
                    name = order.getName();
                     ...
                     if(Strings.isEmptyOrNull(order.getCriteriaAlias())) {
                         name = order.getName();
                     } else {
